...
- [X] 메뉴는 ID, 이름, 가격, 메뉴 그룹, 노출 여부, 메뉴 상품 목록을 갖는다.
- [X] 메뉴를 등록할 수 있다
    - [X] 메뉴는 메뉴 그룹에 속해야 한다.
    - [X] 메뉴에는 하나 이상의 메뉴 상품이 포함되어야 한다.
    - [X] 여러 상품을 하나의 메뉴로 구성 가능하다. (예: 짜장면 2개와 짬뽕 1개를 하나의 메뉴로 묶어 판매할 수 있다)
    - [X] 메뉴 가격은 (상품 가격 * 수량)의 총합보다 작거나 같아야 한다
        - 예) 메뉴 구성: 짜장면 개당 2000원 * 2개, 짬뽕 개당 1000원 * 1개 => 메뉴의 가격은 5000원 이하여야 한다
    - [X] 메뉴의 이름은 비어있거나 욕설이 포함될 수 없다
- [ ] 메뉴 가격을 변경할 수 있다
    - [ ] 메뉴 가격은 비어있을 수 없다
    - [ ] 메뉴 가격은 0원 이상이다
